...
class CURELearner():
    def __init__(self, net, trainloader, testloader, device='cuda', lambda_ = 1e-5, eigen = 5,
                 path='.'):
        '''
    ...
    def freeze(self):
        ...
        Freezes all layers expect the last one!
    ...
    def _train(self, epoch, h):
        ...
        for batch_idx, (inputs, targets) in enumerate(self.trainloader):
            ...
            # sort, indices = torch.sort(outputs, descending=True, dim=1)
            # second_probable_class = indices.T[1]
            ...
            reg = self.hessian_diag_sum(inputs, targets, neg_log_likelihood,
                                        repeat=self.eigen, h=h)
            loss = neg_log_likelihood + self.lambda_ * reg
        ...
        self.writer.add_scalar("Train/regularizer", reg_loss/(batch_idx+1), epoch)
    ...
    def test(self, epoch, h):
        ...
        Testing the model 
        '''
        test_loss, total, reg_loss, net_loss, clean_acc = 0, 0, 0, 0, 0
        ...
        for batch_idx, (inputs, targets) in enumerate(self.testloader):
            ...
            with torch.no_grad():
                ...
                reg = self.hessian_diag_sum(inputs, targets, loss,
                                            repeat=self.eigen, h=h)


            reg_loss += reg.item()
            net_loss += loss.item() + self.lambda_ * reg.item()

        print(f'epoch = {epoch},  clean_acc = {clean_acc/total}, loss = {test_loss/(batch_idx+1)}', \
            f'regularizer = {reg_loss/(batch_idx+1)}')
        ...
        self.writer.add_scalar("Test/regularizer", reg_loss/(batch_idx+1), epoch)
    ...
    def hessian_diag_sum(self, inputs, targets, base_loss, repeat, h):
        diag = 0
        basis = torch.zeros((1, 3, 32, 32)).to(self.device)
        for _ in range(repeat):
            c = np.random.randint(3)
            i = np.random.randint(32)
            j = np.random.randint(32)
            basis[0][c][i][j] = 1
            # print(inputs[0][c][i][j].item())
            out_left = self.net.eval()(inputs + h * basis)
            out_right = self.net.eval()(inputs - h * basis)
            d = 1/h**2 * (self.criterion(out_left, targets)
                          + self.criterion(out_right, targets)
                          - 2 * base_loss)
            diag += d
